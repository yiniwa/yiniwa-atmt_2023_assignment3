...
def main(args):
    ...
    def load_data(split, args, src_dict, tgt_dict):
        # Adjust the extensions for BPE
        src_ext = f'bpe.{args.source_lang}'
        tgt_ext = f'bpe.{args.target_lang}'

        # Build the file paths for source and target
        src_file = os.path.join(args.data, f'{split}.{src_ext}')
        tgt_file = os.path.join(args.data, f'{split}.{tgt_ext}')

        # Check if the files exist and if not, try changing the case of 'bpe'
        if not os.path.isfile(src_file):
            src_file = os.path.join(args.data, f'{split}.{src_ext.upper()}')
        if not os.path.isfile(tgt_file):
            tgt_file = os.path.join(args.data, f'{split}.{tgt_ext.upper()}')

        # Check if the files exist and raise an error if not found
        if not (os.path.isfile(src_file) and os.path.isfile(tgt_file)):
            raise FileNotFoundError(
                f"BPE data files not found for {split} split.")
    ...
    train_dataset = load_data('train', args, src_dict, tgt_dict) if not args.train_on_tiny else load_data(
        'tiny_train', args, src_dict, tgt_dict)
    valid_dataset = load_data('valid', args, src_dict, tgt_dict)
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            if torch.isnan(loss).any():
                ...
                print(src_dict.string(sample['src_tokens'].tolist()[
                      0]), '---', tgt_dict.string(sample['tgt_tokens'].tolist()[0]))
                # print()
